...
export const Dashboard = () => {
  ...
  const monthStats = useMemo(() => ({
    ...
    delivered: filteredPackages.filter(p => p.status === 'Delivered').length,
    inTransit: filteredPackages.filter(p => p.status === 'In Transit').length,
    failed: filteredPackages.filter(p => ['Returned', 'Cancelled'].includes(p.status)).length,
    pending: filteredPackages.filter(p => p.status === 'Pending').length,
    revenue: filteredPackages.filter(p => p.status === 'Delivered').reduce((acc, p) => acc + (p.collectedAmount || 0), 0),
  }), [filteredPackages]);
  ...
  const pieData = useMemo(() => [
    ...
    { name: 'Échoué',     value: monthStats.failed,     color: '#EF4444' },
  ], [monthStats]);
...
INDICATEURS CLÉS
  ...
  • Colis échoués/retournés : ${monthStats.failed}
...
${monthStats.failed > 0
  ? `${monthStats.failed} colis échoués représentent ${monthStats.total > 0 ? Math.round(monthStats.failed / monthStats.total * 100) : 0}% du volume total. Une analyse des causes est recommandée.`
  : `Aucun colis échoué ce mois — excellente maîtrise opérationnelle.`
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          ...(currentUser?.role !== 'Agency Moderator' ? [{ label: 'Échoués', value: monthStats.failed, icon: AlertCircle, bg: 'bg-red-50', text: 'text-red-600' }] : []),
        ].map(s => (
