...
export const PackageMonitoring = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'Pending' as PackageStatus, collectedAmount: 0, shippingFee: 7.000,
    weight: 1, fragile: false, packageName: '', courier: '', paymentStatus: 'Pending' as const,
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    addPackage({ ...formData, shippingFee: formData.shippingFee });
    setIsModalOpen(false);
    setFormData({ shipper: '', recipientName: '', recipientPhone: '', recipientAddress: '', city: '',
      status: 'Pending', collectedAmount: 0, shippingFee: 7.000, weight: 1, fragile: false, packageName: '', courier: '', paymentStatus: 'Pending' });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Annulé', count: filteredPackages.filter(p => p.status === 'Returned' || p.status === 'Cancelled').length, icon: AlertCircle, color: 'text-red-600', bg: 'bg-red-50' },
        ].map(stat => (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nouvel envoi">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-500 uppercase">Frais livraison (TND)</label>
              <input required type="number" step="0.001" value={formData.shippingFee}
