  ...
  const openModal = (shipper?: User) => {
    ...
    if (shipper) {
      ...
      setFormData({
        ...
        shippingFee: shipper.shippingFee ?? 7.000,
        returnFee: shipper.returnFee ?? 0,
      });
    } else {
      setEditingShipper(null);
      setFormData({ companyName: '', name: '', phone: '', email: '', address: '', password: '', selectedAgency: '', pickupFee: 0, shippingFee: 7.000, returnFee: 0 });
    }
    setIsModalOpen(true);
  };
          ...
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  Frais de livraison par défaut pour cet expéditeur.
                </p>
              </div>
              {/* Frais de retour */}
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => setFormData({...formData, returnFee: parseFloat(e.target.value) || 0})}
                    className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                    placeholder="0.000"
                  />
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  Frais appliqués en cas de retour du colis.
                </p>
              </div>
              {currentUser?.role === 'Super Admin' && (
                <div>
